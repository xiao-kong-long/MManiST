...
import argparse
from GraphZoo.graphzoo.utils.train_utils import add_flags_from_config
...
config_args = {
    'training_config': {
        ...
        'log-freq': (100, 'how often to compute print train/val metrics in epochs (type: int)'),
        'eval-freq': (10, 'how often to compute val metrics in epochs (type: int)'),
    ...
    'model_config': {
        ...
        'use_att': (0, 'whether to use hyperbolic attention (1) or not (0) (type: int)'),
        'local_agg': (0, 'whether to local tangent space aggregation (1) or not (0) (type: int)')
    },
    'data_config': {
    ...
    'other_config': {
        'recon': (False, 'whether to return reconstructive features (type: bool)'),
        'exist_epoch':(0, 'existed trainning epochs'),
        'feat_dims':([128], 'feature dims of auto-encoder architecture'),
        'mask_nodes':([], 'mask nodes for permutation'),
        'mask_rate':(0.5, 'mask rate for permutation'),
        'embedding':([], 'embedding'),
        'fusion_model':('concat', 'model of multi view embedding fusion'),
        'method':('HGCNAE', 'method'),
        'tech':('10x Visium', 'data acquire technology'),
        'dataset':('DLPFC', 'dataset'),
        'section_id': ('151676', 'id of section'),
        'num_cluster':(7, 'number of clusters'),
        'combine_model':('consensus', 'how to combine multiple clustering results or embeddings')
    }
}
